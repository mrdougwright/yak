...
const showWelcome = () => {
  ...
  const tips = [
    { icon: "🤖", text: "Ask questions about your codebase", color: "cyan" },
    { icon: "📝", text: "Edit files with natural language", color: "green" },
    { icon: "🚀", text: "Run commands and see results", color: "magenta" },
    {
      icon: "⚙️",
      text: "Create YAK.md for custom instructions",
      color: "yellow",
    },
    { icon: "❓", text: "Type /help or /quit to exit", color: "blue" },
  ];
...
// Chat session header
const showChatHeader = (modelName, chatName) => {
  const modelInfo = chalk.cyan(`🧠 ${modelName}`);
  const chatInfo = chalk.green(`💬 ${chatName}`);
  const header = `${modelInfo} ${chalk.dim("│")} ${chatInfo}`;
