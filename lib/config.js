...
const CONFIG_DIR = path.join(process.env.HOME || process.env.USERPROFILE, '.yak');
...
async function getDefaultModel() {
  try {
    ...
    if (models.length === 0) {
      ...
      const child = spawn('ollama', ['pull', 'gemma3:1b'], { stdio: 'inherit' });
      ...
      return new Promise((resolve) => {
        child.on('close', (code) => {
          if (code === 0) {
            ...
            console.log('❌ Download failed. You can manually run: ollama pull gemma3:1b');
